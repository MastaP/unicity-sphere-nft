...
/** Auto-fit wraps a caption to at most this many lines. */
...
export interface FitOptions {
  ...
  minFontSize: number;
  /** Shrink to the width and wrap to at most `maxLines`; otherwise wrap at `fontSize` as-is. */
  autoFit: boolean;
  maxLines?: number;
  measure: MeasureText;
}
...
export interface CaptionLayout {
  lines: string[];
  fontSize: number;
  /** The text could not be fitted: a line is wider than `maxWidth` or the line cap was exceeded. */
  overflow: boolean;
}
...
/** Collapse runs of whitespace, trim, and apply the uppercase toggle. */
...
/**
 * Greedy word wrap. A single word wider than `maxWidth` still gets a line of its
 * own: breaking inside a word would change the joke, so shrinking is left to
 * the caller.
 */
...
export function fitCaption(text: string, options: FitOptions): CaptionLayout {
  const { maxWidth, measure, autoFit } = options;
  const maxLines = options.maxLines ?? MAX_CAPTION_LINES;
  ...
  if (!autoFit) {
    ...
    return { lines, fontSize: preferred, overflow: !allLinesFit(lines, maxWidth, preferred, measure) };
  }
  ...
  const fitsAt = (size: number): string[] | null => {
    ...
    return lines.length <= maxLines && allLinesFit(lines, maxWidth, size, measure) ? lines : null;
  };
...
export function placeCaption(
  ...
  const height = layout.lines.length * layout.fontSize * LINE_HEIGHT;
